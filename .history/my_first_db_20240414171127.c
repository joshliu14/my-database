...
typedef struct {
    ...
    char *stringValues[MAX_STRING_VALUES];
    int intValues[MAX_NUM_VALUES];
                ...
                if (strcmp(schema -> tables[i].name, token) == 0) {
                    ...
                    for (int j = 0; j < schema -> tables[i].num_fields; j++) {
                        ...
                        if (strcmp(schema -> tables[i].fields[j].type, "STRING") == 0) {
                            for (int k = 0; k < schema -> tables[i].fields[j].fieldLength; k++) {
        ...
        for (int i = 0; i < schema -> num_tables; i++) {
            if (strcmp(schema -> tables[i].name, token) == 0) {
                ...
                for (int j = 0; j < schema -> tables[i].num_fields; j++) {
                    ...
                    trim(token);
                    int length = schema -> tables[i].fields[j].fieldLength;
                    if (length >= MAX_NUM_VALUES) {
                        printf("Too many values.\n");
                        return;
                    }
                    if (strcmp(schema -> tables[i].fields[j].type, "STRING") == 0) {
                        char value[MAX_FIELD_NAME_LENGTH];
                        strcpy(schema -> tables[i].fields[j].stringValues[length], token);
                    } else {
                        int value = atoi(token);
                        schema -> tables[i].fields[j].intValues[length] = value;
                    }
                    schema -> tables[i].fields[j].fieldLength++;
                }   
                token = strtok(NULL, " ,()");
                if (token != NULL) {
                    printf("Too many values.\n");
