...
typedef enum { STRING, INT } DataType;

// Structure for a field in a table
typedef struct {
...
void create_table(Schema *schema, char *table_name, Field *fields, int num_fields) {
        ...
        if (strcmp(token, "TABLE") == 0) {
            ...
            while ((token = strtok(NULL, " ,)"))) {
                if (strcmp(token, "string") == 0) {
                    fields[num_fields++].type = STRING;
                } else if (strcmp(token, "int") == 0) {
                    fields[num_fields++].type = INT;
                } else {
                    strcpy(fields[num_fields].name, token);
                }
            }
            create_table(schema, table_name, fields, num_fields);
