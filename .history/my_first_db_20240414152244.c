...
void execute_query(Schema *schema, char *query) {
    ...
    if (stricmp(token, "create") == 0) {
        ...
        if (stricmp(token, "table") == 0) {
            ...
            while ((token = strtok(NULL, " ,()"))) {
                if (stricmp(token, "string") == 0) {
                    strcpy(fields[num_fields++].type, "string");
                } else if (stricmp(token, "int") == 0) {
    ...
    } else if (stricmp(token, "select") == 0) {
        token = strtok(NULL, " ");
        if (stricmp(token, "*") == 0) {
            token = strtok(NULL, " ");
            ...
            for (int i = 0; i < schema -> num_tables; i++) {
                if (stricmp(schema -> tables[i].name, token) == 0) {
                    printf("Table name: %s\n", schema -> tables[i].name);
                    for (int j = 0; j < schema -> tables[i].num_fields; j++) {
                        printf("%s: ", schema -> tables[i].fields[j].name);
                        if (stricmp(schema -> tables[i].fields[j].type, "STRING") == 0) {
